...
which cannot fail on the vector it polices is not a check:
  ...
  C3  reproduction-- weights @ C, percentiled, reproduces all 72 raw endpoints in
  ...
  C4  publication -- every rendering of those endpoints in artifacts/step9-headline-a.json
                     (6 dp) and artifacts/step9-headline-a.md (4 dp) is reproduced
...
ARM_KEYS = ["W108_s2_finale", "W091_s2_finale", "W091_s2_premiere"]
POPS = ["APPLY", "DERIV"]
...
check("C2_conformance_redraw", bool(np.array_equal(R, Wm)),
      {"generator": "numpy.random.default_rng", "seed": SEED,
       ...
       "emitting_chunk": MAN["chunk_used_here"], "redraw_chunk": 2000,
...
check("C2_negative_control", not bool(np.array_equal(redraw(SEED + 1, 2000), Wm)),
...
check("C3_reproduction_raw_endpoints", exact == checked == 72,
...
m_seed = n_endpoints_matching(endpoints_from(redraw(SEED + 1, 2000).astype(np.float64) @ C))
check("C3_negative_control_different_seed", m_seed == 0,
      {"substitution": f"a full valid replicate set drawn at seed {SEED + 1}",
       "endpoints_that_still_matched": m_seed, "of": 72, "rows_checked": 72,
       "note": "0 of 72 must come back, or the endpoints are insensitive to the draw"})
...
check("C3_negative_control_different_mechanism", m_mech == 0,
      {"substitution": "rng.multinomial(n_frame, uniform, size=B) at the SAME seed",
       "endpoints_that_still_matched": m_mech, "of": 72,
...
json_ok, json_checked, json_bad = 0, 0, []
for path, obj in found:
    if obj["statistic"] == "levels":
        m = re.match(r"\$\.arms\[(\d+)\]\.headline\.(APPLY|DERIV)\..*\.shares\.(\w+)\.ci$", path)
        if not m:
            json_bad.append({"path": path, "why": "level CI at an unrecognised path"})
            continue
    ...
    for end in ("lower", "upper"):
        ...
        if r6(obj[end]) == want:
            ...
            json_bad.append({"path": path, "end": end, "published": obj[end], "recomputed": want})
check("C4a_published_json_renderings", json_ok == json_checked == 54 and not json_bad,
      {"ci_objects_found": len(found), "endpoint_values_checked": json_checked,
       "reproduced": json_ok, "precision": "6 dp", "mismatches": json_bad,
...
in_mov = False
for line in md:
    if line.startswith("| Outcome |") and "95% CI, LEVEL" in line:
        in_level, in_mov, lvl_tbl, row_i = True, False, lvl_tbl + 1, 0
        continue
    if line.startswith("| Arm |") and "95% CI, MOVEMENT" in line:
        in_mov, in_level, row_i = True, False, 0
        continue
    if in_level:
        m = re.search(r"\|\s*\[([\d.]+)%,\s*([\d.]+)%\]\s*\|", line)
        if m:
            md_pairs.append(("level", lvl_tbl * 3 + row_i, float(m.group(1)), float(m.group(2))))
            row_i += 1
...
expected = [(a, p, o) for a in ARM_KEYS for p in POPS for o in OUTCOMES]
md_ok, md_checked, md_bad = 0, 0, []
for kind, ordinal, lo, hi in md_pairs:
    a, p, o = expected[ordinal]
    blk = recomputed[f"{a}|{p}|{o}"]["level" if kind == "level" else "movement"]
    for end, got in (("lower", lo), ("upper", hi)):
        md_checked += 1
        if r4(blk[end]) == got:
            md_ok += 1
        else:
            md_bad.append({"kind": kind, "ordinal": ordinal, "key": f"{a}|{p}|{o}", "end": end,
                           "published": got, "recomputed": r4(blk[end])})
n_lvl = sum(1 for k, *_ in md_pairs if k == "level")
n_mov = sum(1 for k, *_ in md_pairs if k == "movement")
check("C4b_published_md_renderings",
      md_ok == md_checked == 72 and n_lvl == 18 and n_mov == 18 and not md_bad,
      {"level_rows_parsed": n_lvl, "movement_rows_parsed": n_mov,
       "endpoint_values_checked": md_checked, "reproduced": md_ok, "precision": "4 dp",
       "mismatches": md_bad, "rows_checked": md_checked,
       "file": "artifacts/step9-headline-a.md",
       "note": ("the md carries all 18 movements; the JSON carries only the 9 with "
