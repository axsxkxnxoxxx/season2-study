"""Step 7 RERUN on the ADOPTED rule ALT-MATCHED (decisions/0052), namespace `a`. STAGE 1.

THE ADOPTED RULE (decisions/0052 SS1, task-sheet.md Step 7)
  A user-show pair is NOT LIVE if and only if EITHER
    (i)  |A| = 0  AND the account shows no insertion instant after tau1 = [[T0]] + W*24h;  OR
    (ii) |A| >= 1 AND the pair is NOT Continued AND the account shows no insertion instant
         after tau2 = [[T0]] + (W+H)*24h.
  Each null is tested at the instant its own outcome is READ. Never-started at tau1;
  started-and-left at tau2, because the Continued condition it negates is read at tau2.
  Continued is Step 1 SS7 as amended by 0034:
    |A| >= 1  and  F2 in A_H  and  |A_H| >= ceil(0.90 * L2),  A_H read at tau2.

STAGE 1 rebuilds and ASSERTS the two populations, from the Step 5 pair table, and re-asserts
the whole Step 5 waterfall before use:
  DERIV  line 4 (152,126) less D10 -> asserted == 147,370   requires S2 evidence
  APPLY  line 1 (201,900) less D10 -> asserted == 196,654   what Step 8 filters at position 6

D10 (Step 1 SS6): [[T0]] + (max(W,91) + H)*24h <= tau_pull, W = 108, H = 91. Filter order
decisions/0029: ... 4 contamination -> 5 right-censoring -> 6 liveness.
...
PRIOR = os.path.join(ROOT, "processed/step7/bb_a")     # namespace `a` prior run (ALT-BROAD)
...
def main():
    ...
    tau2_max_possible = TAU_PULL          # D10 forces tau2 <= tau_pull at every arm with W<=91?
    # D10: t0 <= tau_pull - (max(W,91)+H)*86400, so tau2 = t0 + (W+H)*86400
    #      <= tau_pull - (max(W,91) - W)*86400 <= tau_pull. Asserted below on the real data.
    assert float(np.nanmax(tau2[apply_])) <= TAU_PULL, "tau2 past the pull instant"
    ...
    # REUSE CROSS-CHECK: the prior namespace-`a` ALT-BROAD run's population must be
    # bit-identical, or reusing its episode table / instants in stage 2 is not licensed.
    prior_ok = None
    pf = os.path.join(PRIOR, "population.npz")
    if os.path.exists(pf):
        q = np.load(pf)
        prior_ok = {
            "line1": bool(np.array_equal(q["line1"], line1)),
            "line4": bool(np.array_equal(q["line4"], line4)),
            "deriv": bool(np.array_equal(q["deriv"], deriv)),
            "apply_": bool(np.array_equal(q["apply_"], apply_)),
            "tau1": bool(np.array_equal(np.nan_to_num(q["tau1"], nan=-1.0),
                                        np.nan_to_num(tau1, nan=-1.0))),
            "tau2": bool(np.array_equal(np.nan_to_num(q["tau2"], nan=-1.0),
                                        np.nan_to_num(tau2, nan=-1.0))),
            "user_idx": bool(np.array_equal(q["user_idx"], p.user_idx.values.astype(np.int64))),
        }
        assert all(prior_ok.values()), f"prior population differs: {prior_ok}"

    summary = {
        "step": 7,
        "what": "Step 7 RERUN on ALT-MATCHED (decisions/0052). GATE. Adopts nothing.",
        "instance": "mm_a",
        "stage": 1,
        "api_calls": 0,
        "rule": ("NOT LIVE iff  (|A|=0 AND no insertion after tau1)  OR  "
        ...
        "tau1_max_observed_utc": str(np.datetime64(int(np.nanmax(tau1[apply_])), "s")),
        "tau2_max_observed_utc": str(np.datetime64(int(np.nanmax(tau2[apply_])), "s")),
        "tau2_never_exceeds_tau_pull": True,
        "populations": {
            "DERIV": {
        ...
        "prior_namespace_a_population_identical": prior_ok,
    }
