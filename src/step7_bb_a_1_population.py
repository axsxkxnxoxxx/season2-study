"""Step 7 RERUN on the ADOPTED rule ALT-BROAD (decisions/0048), namespace `a`. STAGE 1 of 7.

THE ADOPTED RULE (decisions/0048 SS1)
  A user-show pair is NOT LIVE iff BOTH
    (i)  the account shows no insertion instant after that pair's tau1 = [[T0]] + W*24h, AND
    (ii) the pair is NOT Continued.
  Continued is Step 1 SS7 as amended by 0034:
    |A| >= 1  and  F2 in A_H  and  |A_H| >= ceil(0.90 * L2),  read at tau2 = [[T0]] + (W+H)*24h.

STAGE 1 rebuilds and ASSERTS the two populations named at decisions/0046 SS0, from the Step 5
pair table, and re-asserts the whole Step 5 waterfall before use:
...
decisions/0029: ... 4 contamination -> 5 right-censoring -> 6 liveness. D10 runs BEFORE
liveness, so a D10-removed pair is never offered to the rule.
...
def main():
    ...
    # the largest tau1 the population can hold, at ANY arm: D10 forces
    # t0 <= tau_pull - (max(W,91)+H)*24h, so tau1 = t0 + W*24h <= tau_pull - H*24h whenever
    # W <= max(W,91). Used by the calibration-clamp argument in stage 6.
    tau1_max_possible = TAU_PULL - H_DAYS * SEC_PER_DAY
    ...
    # reuse check: the prior ALT run's population must be bit-identical, or the reuse of its
    # episode table in stage 2 is not licensed
    prior_ok = None
    ...
    if os.path.exists(pf):
        ...
        prior_ok = {
            ...
            "t0_floor": bool(np.array_equal(np.nan_to_num(q["t0_floor"], nan=-1.0),
                                            np.nan_to_num(t0f, nan=-1.0))),
    ...
    summary = {
        "step": 7,
        "what": "Step 7 RERUN on ALT-BROAD (decisions/0048). GATE. Adopts nothing.",
        "instance": "bb_a",
        "stage": 1,
        "api_calls": 0,
        ...
        "tau1_max_possible_any_arm_utc": str(np.datetime64(int(tau1_max_possible), "s")),
        ...
        "prior_alt_population_identical": prior_ok,
    }
