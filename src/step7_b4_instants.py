"""Step 7 rerun (instance b4) -- stage 1: per-account DISTINCT insertion instants.

READ ONLY. ZERO network calls.

Spec: decisions/0040 (gate reopened), task-sheet.md Step 7 as corrected,
decisions/0038 (frozen spec), 0037 Sec 4 (gap unit), 0036 Sec 2 (rule shape,
Sec 2.3 second edge case WITHDRAWN), 0021 (insertion time), 0025 (ceiling).

The gap-unit operation, verbatim from decisions/0037 Sec 4:
...
NOT refitted (0038, 0036 Sec 3). The application is verbatim
...
Out: processed/step7/b4/instants.npz  (flat distinct-instant array + offsets)
     processed/step7/b4/instants_meta.json
"""
...
def main() -> None:
    ...
    print(f"calibration knots {len(knot_rid):,}  READ, NOT REFITTED")
    ...
    print(f"calibrated  ({time.time() - t:.1f}s)", flush=True)
    ...
    u_s = user[order]
    t_s = tau[order]
    print(f"sorted  ({time.time() - t:.1f}s)", flush=True)
    ...
    t_k = t_s[keep]
    n_dist = len(t_k)
    print(f"distinct instants {n_dist:,}  ({time.time() - t:.1f}s)", flush=True)
    ...
    meta = {
        ...
        "calibration": {
            ...
            "clamping_note": (
                ...
                "single instant each by the exact-tie rule. JUDGEMENT CALL: the spec does not "
                "state how to treat records outside the fitted rid range. Reported, not repaired "
                "-- the curve is a required input and is not refitted."
            ),
        },
