"""Step 7 RERUN ON THE ADOPTED RULE (instance b, namespace alt2_b) -- stage 2.

Outcome assignment on the line-1 (201,900) population at EVERY W arm. This
supplies conjunct (b) of the adopted rule -- |A| = 0 -- and the three shares.
...
|A| is computed INDEPENDENTLY of liveness -- it is a function of T0, W, E2 and
the episode records only. That independence is what makes the two predicates
row-local and commuting (decisions/0046 Sec 5).

Also recorded here, because 0046 Sec 1 makes a factual claim about it that this
gate must confirm or refute: for every line-1 pair, whether it has ANY S2
record anywhere in the sweep, and whether it has any in-E2 S2 record.

ZERO network calls. Reads only.

Out: processed/step7/alt2_b/outcomes.npz, processed/step7/alt2_b/stage2.json
"""
...
def main() -> None:
    t = time.time()
    prov: dict = {"instance": "data-scientist-b", "namespace": "alt2_b", "stage": 2,
    ...
    # any S2 record anywhere (timestamped or not, in E2 or not) -- 0046's claim set
    ...
    prov["line1_pairs_with_NO_in_E2_S2_record"] = int(n - has_in_e2.sum())
    ...
    for W in W_ARMS:
        ...
        assert (nA <= pz["L2"]).all()         # distinct episodes never exceed L2
        ...
        per_arm[str(W)] = {
            ...
            "never_started_pairs_with_no_S2_record_anywhere":
                int(((~started) & ~any_s2_record).sum()),
        }
