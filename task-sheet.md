...
- [ ] **Negative-lag report (D2), split THREE WAYS, not two: the S2 finale binds, the S1 completion
      binds, or BOTH bind.** Human Lead ruling, 2026-08-13 (`0070`). **168 pairs have both terms binding — ***AND THE POPULATION IS UNSTATED HERE, WHICH IS THE DEFECT.*** Red Team seventh pass, N2. **The two arms read it on populations 23,453 pairs apart** — one on the position-5 set (196,654), the other on line 1 (220,107) in its waterfall **and on 196,654 in its own invariants**, so one arm carries the same integer under two populations across its own two deliverables. **168 cannot be correct on both.** **STATE THE POPULATION AT THE POINT OF USE AND MEASURE IT ON BOTH** — this is the standing provenance rule (`0047`, `0078` §2), not a new requirement, and the number itself has never been measured on both. **The dual diff reads 168 against 168 as agreement**, which is why six passes did not see it. Retained for the record: 168
