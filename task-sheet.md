...
- [ ] **Exclusions: 0 pairs on DERIV, 604 pairs on APPLY** at `W = 108`, both **measured** by both arms. A Step 7 instance reporting 0 on DERIV and 604 on APPLY is correct and the two are **not** a divergence.
...
- [ ] **Report the exclusion count per `W` arm on APPLY**, so the `W`-coupling is visible (`0044` §1.2, corrected to APPLY by `0046`). **APPLY: 485 / 494 / 554 / 575 / 603 / 604 / 664 / 716** at `W` = 38 / 46 / 77 / 91 / 107 / 108 / 150 / 213.
- [ ] **D10 IS RE-DERIVED AT EACH ARM. Name the reading; do not freeze it.** Human Lead ruling, 2026-08-13 (`decisions/0047`). Right-censoring is `⟦T0⟧ + (max(W, 91) + H) × 24h ≤ τ_pull`, which **contains `W`**, so the censored population differs per arm. **Freezing D10 at `W = 108` gives 632 / 684 / 753 / 881 at the upper arms instead** — a different table. **An arm table that does not name the reading is not reproducible**, which instance A demonstrated by producing both.
...
- [ ] **Expect 604 liveness exclusions at position 6, `W = 108`, and treat a mismatch as a POPULATION defect before an implementation one.** Human Lead ruling, 2026-08-13 (`decisions/0047`). Step 7 measured 604 on the application population, but built it from the Step 5 pair table rather than through positions 1–5. **A different count most likely means the frame join, the `L2 = 1` exclusion or the censoring differs — not that the liveness rule was coded wrong.** Check the population first.
