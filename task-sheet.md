...
- [ ] ~~**The bootstrap is unspecified and Step 9's CIs are not diffable until it is fixed** — `B`, seed
      and levels-vs-movements all differ between the arms.~~ ***THE BOOTSTRAP IS FULLY SPECIFIED:
      `B` = 10,000, seed 20260818, unit = account (`0103`), statistic = BOTH levels and paired
      movements (`0118`).*** ***AND "all differ between the arms" WAS NEVER TRUE OF THE STATISTIC***
      — corrected, not marked, at `0118` §2: **arm `a` published both objects on both runs.** `B` and
      the seed did differ. The schema must still **record which bootstrap settings produced each CI**,
      so the settings are visible in the output rather than silent, **and both statistics must be
      findable there.**
...
- [ ] **ATTACH CONFIDENCE INTERVALS — AND THE BOOTSTRAP IS UNSPECIFIED. THIS BLOCKS STEP 9, NOT STEP 8.** Human Lead ruling, 2026-08-13 (`0056`). ~~**The two Step 7 arms diverged on all three of `B`, seed and statistic** — **A: B = 4,000, seed 20260813, on the movements; B: B = 2,000, seed 20260814, on the levels**~~ ***CORRECTED, NOT MARKED, 2026-08-19 (`0118` §2): THE STATISTIC HALF WAS NEVER TRUE.*** **Arm `a` published BOTH levels and paired movements** — `artifacts/step7-liveness-mm-a.json:1018`, and the gate-closing `bb` run computes both as well. **The arms diverged on TWO elements, `B` and the seed, not three** — **A: B = 4,000, seed 20260813; B: B = 2,000, seed 20260814** — and a dual step whose CIs are built two different ways **produces a divergence that proves nothing.** **`0052` §6 recorded this as "unreconciled and now specified." It was never specified: the string "bootstrap" appears ZERO times in `task-sheet.md`, `CLAUDE.md` and all four pipeline agent files.** That claim is **struck in place** — it is a completed action asserted and not taken, the same class as `0055` §5a. ~~**Specify all three before Step 9 launches**~~ ***ALL FOUR ARE NOW SPECIFIED AND IDENTICAL FOR BOTH ARMS*** *(**FOUR listed, and `0056` §159's list named THREE** — `B`, the seed and the statistic; **the unit was fixed separately by `0044`/`0103` §2.** The count is stated as four because four are enumerated. Corrected 2026-08-19, `0120`, found by the arm on the v1.7.0 run — **E3's own class, in the sentence `0118` made canonical.**)*: the resampling unit is the **account** (clustered, per `0044`), `B` = **10,000** and the seed = **20260818** (`0103`), and the statistic is ***BOTH LEVELS AND PAIRED MOVEMENTS, both labelled, neither presented as the design*** (`0118`). **The requirement exists so the diff compares like with like, and both arms producing both objects satisfies it fully** — a divergence on either object is then a real divergence rather than a design difference. **Neither object may be dropped: on APPLY the never-started LEVEL is 1.09 pp wide against a 0.098 pp MOVEMENT, a factor of 11**, so a reader not told which one they are reading is wrong by an order of magnitude — **the same reasoning as publishing the floor and the ceiling rather than a point.** ***Step 9's CIs are diffable and Step 9 is not blocked on the bootstrap.*** **The canonical wording both writer files carry is the marked `BOOTSTRAP-STATISTIC` block, and `src/check_surfaces.py::scan_statistic_declaration()` fails if the two copies differ.**
