...
**Owner:** Data Scientist, dual implementation
**Mode:** GATE. **APPROVED 2026-08-13 (`decisions/0042-step7-liveness-gate-parameter-free.md`).**

> ## THE APPROVED RULE — there is no threshold
>
...
- [ ] **Exclusion counts, BOTH populations — Step 8 will produce the second and it is NOT a divergence.** Human Lead ruling, 2026-08-13 (`decisions/0045`).
      **Derivation population** (Step 5 line 4 less D10, **147,370**): **751 pairs from 166 accounts, 0.51%.**
      **Application population** (line 1 less D10, **196,654** — what Step 8 filters at position 6): **1,355 pairs from 276 accounts.**
      **The two differ because line 4 requires S2 evidence.** The extra 604 are pairs with **no S2 record anywhere**, which line 4 removes by construction and Step 8's population does not. **A Step 8 instance reporting 1,355 is correct.**
...
- [ ] **Compute the bound ONLY over the liveness exclusions that are scored NEVER STARTED — not over all of them.** Human Lead ruling, 2026-08-13 (`decisions/0045`), Option C. **On the application population that is [16.7789%, 17.0355%], width 0.257 pp.**
      **Why the old form is not a bound.** It treated every excluded pair as a decliner, but **all 751 of the derivation-population exclusions have positive in-window S2 evidence and 652 are confirmed continuers.** Treating a confirmed continuer as a decliner is not conservative — it is arithmetic on a set chosen for a reason unrelated to the uncertainty being bounded, and it put the **ceiling outside the feasible set**: 6.7151% against an unfiltered 6.2055% on the derivation population, **0.62× the clustered sampling width of the share it bounds.**
      **Restricting to the never-started exclusions makes both endpoints attainable**, and lands within **0.001 pp** of the width the rejected ALT rule would have given, while touching no rule, population, filter order or agent file.
