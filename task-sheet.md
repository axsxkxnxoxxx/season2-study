...
**Owner:** Data Scientist, dual implementation
**Mode:** GATE. **ALT-BROAD restored 2026-08-14 (`decisions/0054`) after ALT-MATCHED was reverted. NOT approved; seven Red Team HOLDs.** Both arms have run on ALT-BROAD and on ALT-MATCHED; all figures for both are on record.
...
- [ ] **Report the exclusion count per `W` arm on APPLY**, so the `W`-coupling is visible (`0044` §1.2, corrected to APPLY by `0046`). **ALT-MATCHED on APPLY: 537 / 550 / 633 / 664 / 701 / 703 / 789 / 864** at `W` = 38 / 46 / 77 / 91 / 107 / 108 / 150 / 213. **The started-and-left component alone runs 52 / 56 / 79 / 89 / 98 / 99 / 125 / 148 — a factor of 2.85, growing faster than the rule's own 1.5× coupling. Report it separately.**
...
- [ ] **Compute a SECOND bound, on the started-and-left share, over all 703 exclusions AND WIDENED TO COVER THE 90.** **On APPLY: [9.6372%, 10.0405%], width 0.4033 pp, both endpoints on 196,654.** Human Lead ruling, 2026-08-14 (`decisions/0054`).
      **Why the floor is 18,952 and not 19,042.** The 90 started-and-left pairs whose last insertion falls inside `(τ1, τ2)` **could not produce evidence dated after that instant**, so they may in truth be Continued. **The floor must admit that**, and 18,952 / 196,654 = **9.6372%** is what does. `0049`'s **[9.6830%, 10.0405%]** did not, and `0052` §4 declined to widen it on the ground that doing so *"would have been the fifth consecutive bound with a non-covering endpoint"* — **exactly backwards: widening is what makes it covering.**
      **Report [9.6830%, 9.7333%] only as a labelled conditional sub-interval**, never as the bound.
      **The Continued ceiling moves with it: 144,933 / 196,654 = 73.6995%**, since the same 90 may be Continued.
- [ ] **STATE THAT THE TWO CEILINGS CANNOT BOTH HOLD.** Human Lead ruling, 2026-08-14 (`decisions/0050`). **THERE ARE THREE CEILINGS, NOT TWO, AND NONE OF THEM CAN HOLD WITH THE OTHERS.** Never-started **16.9704%** (33,373), started-and-left **10.0405%** (19,745), **Continued 73.6995%** (144,140 + 703 + the 90) — **sum 100.7104% on 196,654.**
