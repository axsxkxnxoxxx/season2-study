...
- [ ] **Exclusions: 99 pairs on DERIV (73 accounts), 703 on APPLY (216 accounts)** at `W = 108` — **604 never-started plus 99 started-and-left**. Under the superseded ALT the counts were 0 and 604. A Step 7 instance reporting 0 on DERIV and 604 on APPLY is correct and the two are **not** a divergence.
      **How the rule selects, corrected 2026-08-13 (`decisions/0047`):** **conjunct 2 (`|A| = 0`) narrows APPLY 196,654 → 33,373; conjunct 1 (no insertion after `τ1`) narrows 33,373 → 604.** **Conjunct 1 does most of the work**, which is why the count moves with `W`. **The 604 are NOT "exactly the pairs with no S2 record anywhere"** — APPLY holds **23,260** such pairs and **22,656 stay live**. Subset, not equality.
...
- [ ] **D10 IS RE-DERIVED AT EACH ARM. Name the reading; do not freeze it.** Human Lead ruling, 2026-08-13 (`decisions/0047`). Right-censoring is `⟦T0⟧ + (max(W, 91) + H) × 24h ≤ τ_pull`, which **contains `W`**, so the censored population differs per arm. **Freezing D10 at `W = 108` gives 632 / 684 / 753 / 881 at `W` = 125 / 150 / 180 / 213 instead — and 125 and 180 are not in the mandated grid, so only 684 (`W` = 150) and 881 (`W` = 213) are comparable to it** — a different table. **An arm table that does not name the reading is not reproducible**, which instance A demonstrated by producing both.
...
- [ ] **Assert invariant: filter counts decrease monotonically — CODED AS `>=`, NOT `>`.** Human Lead ruling, 2026-08-13 (`decisions/0047`). **Decrease is strict at line 6 on the application population and only NON-STRICT on the derivation population**, where the liveness exclusion set is empty. **An implementation asserting strict decrease everywhere fails on correct data.** Both arms found this independently.
...
- [ ] **Compute a SECOND bound, on the started-and-left share, over the 99 started-and-left exclusions.** Human Lead ruling, 2026-08-14 (`decisions/0048`). **No such bound exists today**, and ALT-BROAD is what makes it available: the 99 are pairs whose *exit* rests on a null, so the same reasoning that bounds never-started applies to them. Report it on **one denominator**, per the standing rule.
...
- [ ] **The liveness filter moves the result very little, and Step 9's bounds rest on it.** **CORRECTED 2026-08-14 (`decisions/0048`) — this bullet described PF-LIMIT on DERIV as the approved rule.** Under **ALT-BROAD**: **99 exclusions on DERIV from 73 accounts, 703 on APPLY from 216** — 604 never-started plus 99 started-and-left. On APPLY the shares move **−0.2558 / +0.2258 / +0.0300 pp** for the never-started component; the 99 move the started-and-left share by roughly **0.05 pp**. **The superseded figures — 0.032 / 0.023 / 0.009 pp, 751 pairs of 147,370 from 166 accounts — were PF-LIMIT on DERIV and must not be restated.** Step 9's bounds are computed on these sets, so **they are narrow because the filter is small, not because the inference is tight.**
