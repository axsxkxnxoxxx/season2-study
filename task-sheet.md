...
- [ ] ~~Derive the threshold independently. Do not use W as an input to the derivation.~~ **WITHDRAWN 2026-08-13 (`decisions/0038`).** It was **unsatisfiable** after `0037`: any bracketing-gap reference distribution is selected by `τ1`, which contains `W`. **The threshold IS a function of `W`, and the spec now says so rather than requiring the impossible.** Consequence for Step 13 below.
...
- [ ] **THE DERIVATION AND APPLICATION POPULATIONS MUST BE IDENTICAL.** Derive on the 152,126 and apply to the 152,126. Instance A demonstrated that deriving on 128,099 and applying to 201,900 delivers **2.28%, not 1%** — **a milder recurrence of the exact calibrate-on-one-distribution-apply-to-another defect `0037` withdrew.** A calibrated rate is only calibrated on the population it was calibrated on.
- [ ] **WEIGHTING: ONE GAP PER PAIR.** Human Lead ruling, 2026-08-13. Not one per distinct `(account, gap)`, which both arms flagged as the largest single lever in the step — it moved the threshold to 159 d in one arm and 202 d in the other. The rule is applied per pair, so the reference is weighted per pair.
...
- [ ] **STATE THE INERTNESS PLAINLY IN THE DELIVERABLE.** **The measured-gap test does 3.45% of the exclusions and `0036` §2.3's evidence-absence edge cases do 96.55%**, and **this holds across every percentile from the 90th to the 99.9th.** A reader must not take the threshold to be doing work it is not doing.
...
- [ ] **THE TEST APPLIES TO THE GAP BRACKETING `τ1`, NOT TO EVERY GAP IN THE SWEEP.** Human Lead ruling, 2026-08-13 (`decisions/0036-step7-threshold-and-shape.md`). For each pair, take the pair's own `τ1 = ⟦T0⟧ + W × 24h`, find the **last insertion instant at or before `τ1`** and the **first insertion instant after `τ1`** on that account, and test **that one gap** against the threshold.
      ...
      **Edge cases, stated so two instances resolve them alike.** If there is **no insertion instant after `τ1`**, the gap is open-ended and the pair is **not live**. If there is **no insertion instant at or before `τ1`**, the account has no pre-`τ1` evidence and the pair is **not live**. Both are counted and reported separately from pairs failing on a measured gap.
