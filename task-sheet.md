...
- [ ] ~~Derive the threshold independently. Do not use W as an input to the derivation.~~ **WITHDRAWN 2026-08-13 (`decisions/0038`).** It was **unsatisfiable** after `0037`: any bracketing-gap reference distribution is selected by `τ1`, which contains `W`. **The threshold IS a function of `W`, and the spec now says so rather than requiring the impossible.** Measured on the frozen population the coupling runs **576 → 697 days** across the arms; the 408 → 576 and 885 → 973 spans first recorded here were measured on other populations and are superseded (`decisions/0040`). Consequence for Step 13 below.
- [ ] Plot the distribution of gaps between consecutive **insertion** instants per user, per the two items above
- [ ] **THE GAP SEQUENCE IS DEFINED ON DISTINCT INSERTION INSTANTS, AND THIS IS THE EXACT OPERATION.** Human Lead ruling, 2026-08-13 (`decisions/0037-step7-reference-distribution-and-gap-unit.md`), resolving an ambiguity that produced a real divergence between the two arms — one read 4 days from it, the other 7. Both readings were faithful to the old wording, which is the defect.
      **The operation, stated so two instances cannot read it differently:** for each account, take the insertion instant of **every** record in its sweep; **sort ascending**; **collapse runs of EXACTLY equal instants to a single instant** — exact float equality only, **no rounding, no bucketing to any resolution, no per-day or per-second collapse**; then take the **consecutive differences** of that sorted distinct sequence. Each difference is one gap.
      **What this is not.** It is **not** one gap per consecutive pair of *records* — that reading double-counts a batch insert as many near-zero gaps and is withdrawn. It is **not** a dedupe at any rounded resolution — that reading is unreproducible because the resolution is unstated.
      **A sub-second gap between two genuinely distinct instants is a real gap and is retained.** Only exact ties collapse.
- [ ] **Each gap is a continuous instant difference.** Human Lead decision, 2026-08-12 (`decisions/0029-step7-threshold-rule-and-w-propagation.md`). Not claimed `watched_at`, and not floored to whole days before the percentile is taken. This follows from the insertion-time ruling above: if liveness is evidence of the account being alive, the gap between two pieces of that evidence is the gap between when they were *written*.
- [ ] **THE REFERENCE DISTRIBUTION IS THE BRACKETING-GAP DISTRIBUTION ITSELF, NOT THE POOLED ONE.** Human Lead ruling, 2026-08-13 (`decisions/0037`), **withdrawing `0036` §1's basis.** `0036` §1 justified the 99th percentile as "one in a hundred." **That is true of a uniformly drawn gap and false of the gap this rule tests.** The threshold was a percentile of the **pooled** gap distribution while the test selects the gap **bracketing `τ1`**, which is **length-biased** — a gap of length `L` covers `L` of calendar time, so its chance of containing a fixed instant is proportional to `L`. Both Step 7 instances measured the consequence independently and agreed. **The figure is 34.12% on the 152,126** — the 37.4% first recorded here was measured on the 201,900 and is superseded (`decisions/0040`). Either way it is not 1%.
      **Take the percentile on the bracketing-gap distribution itself**, so the stated rate is the rate the rule delivers.
- [ ] **THE REFERENCE POPULATION IS THE 152,126 — waterfall line 4.** Human Lead ruling, 2026-08-13 (`decisions/0038`), adopting instance A's own counter-argument against its proposal. **The 152,126 → 128,099 filter concerns the FIRST S2 WATCH, which plays no part in `τ1` or in liveness**, so conditioning the liveness reference on it excludes pairs for a reason unrelated to what liveness measures. The 201,900 and 178,165 lines carry contaminated `T0`, and `τ1` is built from `T0`.
- [ ] **THE DERIVATION AND APPLICATION POPULATIONS MUST BE IDENTICAL — AND THAT MEANS DERIVING AFTER D10, NOT BEFORE.** Amended 2026-08-13 (`decisions/0040`). `0038` ranked this requirement first and the run still violated it: **liveness is applied at Step 8 position 6, AFTER right-censoring at position 5**, so the application population is the **152,126 less D10** and both arms derived on the uncensored set. **Derive on the post-D10 population and apply to it.** Two consequences: the populations become genuinely identical, and **the open-ended share falls to 0.685%**, which makes the 99th percentile over the extended set finite and **removes the need for edge case (i) to be a separate ruling at all**. Instance A demonstrated that deriving on 128,099 and applying to 201,900 delivers **2.28%, not 1%** — **a milder recurrence of the exact calibrate-on-one-distribution-apply-to-another defect `0037` withdrew.** A calibrated rate is only calibrated on the population it was calibrated on.
- [ ] **WEIGHTING: ONE GAP PER PAIR.** Human Lead ruling, 2026-08-13. Not one per distinct `(account, gap)`, which both arms flagged as the largest single lever in the step — it moved the threshold to **190 d on the frozen population** — the 159 d and 202 d first recorded here were measured on the 128,099 and 201,900 respectively and are superseded (`decisions/0040`). The rule is applied per pair, so the reference is weighted per pair.
- [ ] **STATE THE QUOTA PROPERTY PLAINLY IN THE DELIVERABLE. Do not argue it away.** Taking the percentile on the distribution the test applies to means **the level is set by the exclusion rate rather than by any feature of the data**: choosing `p` mechanically fixes the exclusion rate at `100 − p`% of measured-gap pairs. **That is the price of a calibrated rate, and it is disclosed.** Both arms found this independently; `0036` §1's conservative-direction argument survives and still points up, but it no longer identifies a level.
- [ ] **STATE THE INERTNESS PLAINLY IN THE DELIVERABLE.** **CORRECTED 2026-08-13 (`decisions/0040`).** The figures first written here — 3.45% / 96.55% *"across every percentile from the 90th to the 99.9th"* — were **the 201,900 line's**, and the invariance claim was **arithmetically impossible**: the edge-case count is constant in the percentile while the gap-test count is `100 − p` of the measured-gap set, so the share **must** move. **Measure and state the split on YOUR population at the adopted percentile, and state no invariance.** For reference, the superseded run measured 5.37% / 94.63% on the 152,126 at the 99th, ranging 36.5% → 0.4% across the 90th–99.9th — **but that run derived before D10 and included the 18,250 now returned, so both figures change.** A reader must not take the threshold to be doing work it is not doing.
- [ ] **Propose the threshold and report it; do not adopt it.**
      **`0036` §2 — the rule's shape — STANDS UNCHANGED.** Only §1's basis is withdrawn. The whole-sweep compounding argument is untouched and was corroborated by both arms.
- [ ] **The percentile remains the 99th** unless the Human Lead rules otherwise on seeing the corrected reference distribution. The conservative-direction argument is unaffected: a false-dead removes a pair, and the liveness exclusion already biases the never-started share **down** (Step 14, bias 2).
- [ ] *(WITHDRAWN 2026-08-13, retained for the record)* ~~**THE THRESHOLD IS THE 99th PERCENTILE of that observed gap distribution.**~~ Human Lead ruling, 2026-08-13 (`decisions/0036-step7-threshold-and-shape.md`). **At the 95th, one ordinary gap in twenty trips the threshold; at the 99th it is one in a hundred.** A false-dead removes a pair, and the liveness exclusion already biases the never-started share **down** (Step 14, bias 2), so **the conservative direction is the higher percentile.** The prior wording — "set the threshold well beyond the normal gap" — is **withdrawn**: it named no feature and no number, and it is the same defect as Step 6's "the percentile where the curve flattens", which cost a full dual run and produced two honest answers 61 days apart (`decisions/0024`).
- [ ] **Round the threshold UP — take the ceiling of the percentile.** Per `decisions/0025`. Two reasons, and they agree. **(a) It delivers the percentile it claims.** If the test excludes a gap at or above the threshold, then a threshold floored from 43.2 to 43 declares dead an account whose gap of 43.1 days is *inside* the stated percentile — the same off-by-one that flooring produced at Step 6. **(b) It is the conservative direction.** A higher threshold declares fewer accounts dead, and the liveness exclusion already biases the never-started share **down** (`task-sheet.md` Step 14, bias 2); a lower threshold worsens a bias already on the record.
- [ ] State where and why
- [ ] **Liveness stays anchored at `τ1`, and `τ2` plays no part in it.** Human Lead decision, 2026-08-12 (`decisions/0034`). Outcome assignment now happens at **two** instants, so an instance could reasonably re-anchor liveness at `τ2`. **It must not.** Liveness licenses trusting a null and the null is `|A| = 0`, which is tested at `τ1`. Written here so no Step 7 instance re-anchors it.
- [ ] **THE TEST APPLIES TO THE GAP BRACKETING `τ1`, NOT TO EVERY GAP IN THE SWEEP.** Human Lead ruling, 2026-08-13 (`decisions/0036-step7-threshold-and-shape.md`). For each pair, take the pair's own `τ1 = ⟦T0⟧ + W × 24h`, find the **last insertion instant at or before `τ1`** and the **first insertion instant after `τ1`** on that account, and test **that one gap** against the threshold.
      **Why not the whole sweep.** A whole-sweep test **compounds the false-dead rate with the number of gaps**: at the 99th percentile an account with **50 logged gaps trips it roughly 39% of the time by chance** — `1 − 0.99⁵⁰ = 39.5%`. That is a property of the rule's **shape**, and **no choice of percentile fixes it**; raising the percentile only moves the count at which the compounding bites.
      **Why this gap.** **Liveness licenses trusting a null, and the null is `|A| = 0` tested at `τ1`.** What must be established is that **the account was alive at that moment** — not that it never went quiet across its whole history. An account that vanished for two years in 2019 and was logging normally around `τ1` is evidence *for* the null, not against it.
      **Edge cases. AMENDED 2026-08-13 (`decisions/0040`) — the second one is WITHDRAWN.**
      **(i) No insertion instant after `τ1`:** the gap is open-ended and the pair is **not live**. Counted and reported separately from pairs failing on a measured gap. **Note this branch nearly vanishes once derivation runs after D10** — the open-ended share falls to **0.685%**, so the 99th percentile over the extended set is **finite** and an infinite gap fails a finite threshold on its own.
      **(ii) ~~No insertion instant at or before `τ1` → not live.~~ WITHDRAWN. It contradicted an approved gate.** `0021`, from **gate 2 of 5**, rules that **any record inserted after the window closed proves the account was alive, whatever date it claims.** Every pair in that bucket has insertion instants after `τ1` by construction — zero accounts have no instants, and the minimum is three gaps per account — so `0021` rules them **live** while this bullet ruled them dead. **`0036` is not a gate and cannot override one.** **The 18,250 pairs are returned to the population**, and an account with insertion activity after `τ1` is **live**.
      **Why `0021` is also substantively right here:** for a pair whose account began after `τ1`, the entire history is an import, and **whether S2 appears in that import is exactly the outcome** — absence there is *more* informative than for a real-time logger, not less.
- [ ] Write the resulting rule: a **user-show pair** counts as live if the account shows logged activity after that pair's clock start plus **`W = 108 days`** (approved 2026-08-12, `decisions/0026-step6-window-w-gate.md`; the Step 6 deliverables predate the ceiling ruling and state 107 and 107.7135 — **neither is the adopted value**), with gaps under the threshold. Liveness **evidence** is account-wide — the whole sweep, other shows and movies included, not restricted to the show under study — but the **test** is clock-start-relative and clock start is pair-specific, so **liveness is a pair-level filter**. One account can be live for one show and not for another. Do not drop a user wholesale on a liveness test.

- [ ] **APPROVED 2026-08-13 at 632 DAYS** (`decisions/0039-step7-liveness-gate.md`). Raw 99th 631.8031, ceiling per `0025`, on the 152,126 with one gap per pair. **Report it as 632 d with an account-clustered 95% interval of [528, 787] — NEVER as a bare point estimate.** The i.i.d. interval [632, 645] **overstates precision roughly twentyfold**: gaps within an account are not independent, and **34.4% of pairs share their bracketing gap value with another pair**, largest tie group 298. **Same treatment as `W = 108 ± 18` show-clustered.**
- [ ] **THE REFERENCE SET IS THE EXTENDED SET — measured-gap pairs PLUS open-ended gaps entered as `+∞`.** Human Lead ruling, 2026-08-13 (`decisions/0041`), **provisional pending the Step 9 sensitivity test.** ~~The reference set is the 129,630 measured-gap pairs; open-ended gaps cannot be treated as infinite — they are 3.17% of the extended set, so a 99th percentile taken over it would itself be infinite.~~ **That restriction came from the suspended `0039` §5 and reasoned OPPOSITE to `0040` §2**, which is what makes the percentile finite: **after D10 the open-ended share is 0.5812%**, well under 1%.
      **An absent successor instant enters the reference as `+∞`. This is a CONSTRUCTION ruling, and it is what moves the threshold** — 632 d on the measured-only set against 1,293 d on the extended set, with 975 d a third reading. It is not an exclusion ruling: an infinite gap fails any finite threshold on its own, so edge case (i) is not needed to exclude anything, only to define how open-ended enters.
      **DEGENERACY CAVEAT, which must be carried wherever the threshold is:** above the **99.4188th** percentile the extended-set percentile is **itself infinite** and the rule collapses into edge case (i) alone. At `W = 108` that is **0.25%** of bootstrap replicates; **at `W = 213` it is 2.80%.** **Report the realised rate against both denominators**: 0.9843% of measured-gap pairs, 15.63% of all pairs.
- [ ] **Approved counts at `W = 108` on the 152,126:** live **128,354**; not live measured gap **1,276**; no instant after `τ1` **4,246**; no instant at or before `τ1` **18,250**.
- [ ] **~79% of the "no instant after `τ1`" bucket is right-censoring, not silence** — only about **880 accounts genuinely went dark**. Step 7 derives on an **uncensored** population, so the bucket is inflated by pairs D10 removes at Step 8, where `0029` puts right-censoring at position 5 and liveness at 6.
...
- [ ] **TEST WHETHER THE HEADLINE IS SENSITIVE ACROSS THE LIVENESS THRESHOLD'S CLUSTERED INTERVAL.** Human Lead ruling, 2026-08-13 (`decisions/0040`). Recompute the headline at **both endpoints of the account-clustered interval** for whatever threshold Step 7 lands on, alongside the point value. **If the headline is insensitive across that band, the threshold is deleted rather than published with an interval** — the honest rule becomes "the account has insertion evidence bracketing `τ1`", with no free parameter. **If it is sensitive, the threshold is load-bearing and the band must be propagated into the headline as a sensitivity range**, which no step currently does. Zero API calls; answerable on cached data.
...
- [ ] **Step 7's liveness threshold is derived on a narrower population than it is applied to, and it under-delivers its stated rate.** The threshold is derived on the Step 5 line-4 population less D10 (**147,370**) because the lines above carry contaminated `T0` and `τ1` is built from `T0` (`decisions/0038` §2). Step 8 applies liveness at position 6 to the **analysis population less D10 — 196,654**, a strict superset. **Measured: the rule delivers 1.4418% exclusion against a stated 1%.** Same defect class as the 2.28% example `0038` §2.1 names, one step milder. **Not repairable inside Step 7** — re-deriving on the 201,900 is forbidden, and restricting Step 8 to line 4 would change the analysis population to suit a diagnostic. (`decisions/0041` §3)
