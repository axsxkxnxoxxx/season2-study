...
- [ ] **Compute a SECOND bound, on the started-and-left share, over ALL exclusions AND WIDENED TO COVER THE CHANNEL PAIRS — ON BOTH POPULATIONS.** Human Lead ruling, 2026-08-14 (`decisions/0054`; **DERIV widened by `decisions/0055`**). **EVERY ENDPOINT STATES ITS POPULATION AT THE POINT OF USE, and APPLY and DERIV are published side by side.**
      ...
      **Report [9.6830%, 9.7333%] only as a labelled conditional sub-interval** — that is an **APPLY** figure, never the bound. **On DERIV the bound and its conditional sub-interval COINCIDE**, because the never-started exclusion component is 0 there; say so where it is published.
