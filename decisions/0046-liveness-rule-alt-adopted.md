...
| | |
| :--- | :--- |
| **Decision** | **ALT is ADOPTED.** A pair is **not live iff BOTH** the account shows no insertion instant after `τ1` **and** `|A| = 0`. `0045`'s ALT rejection is **withdrawn** — it was made on the population where ALT is zero by construction. The Step 9 bound is **[16.7146%, 16.9704%] on APPLY**. |
...
| **Status** | Closed. **Step 7 reruns on ALT. The gate is OPEN.** |
...
**Exclusions: 0 on DERIV, 604 on APPLY.** The DERIV zero is **forced by construction** — line 4
requires S2 evidence, so no line-4 pair can have `|A| = 0` and no S2 record. **The 604 on APPLY are
exactly the pairs with no S2 record anywhere.**
...
**Under the adopted rule: [16.7146%, 16.9704%] on APPLY, width 0.2558 pp.**
...
**Step 7's own dual run cannot exercise the rule.** On DERIV the exclusion set is empty, so the diff is
`0 = 0`. **The rule is first exercised at Step 8**, on APPLY. That is a real weakness of the adopted
