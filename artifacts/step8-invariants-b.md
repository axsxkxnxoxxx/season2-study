...
**All invariants pass: True.** For **6 of the 9** — the pure code checks — that statement says only that the code computed what it was told to. It is **not** evidence for the liveness rule, for the outcome definition, or for any published share. **3 could have failed on data as specified: §7 and §8, the two DATA CHECKS, and §5, the clock start** — which is a code check by construction but **recomputes the first-pass S1 completion date INDEPENDENTLY**, and two implementations can disagree on real records. **What they found is reported in full below rather than as a tick.** ***Every number in this paragraph is DERIVED FROM THE `label` FIELD of the emitted invariants, not typed***, so the prose cannot drift from the set it describes.

### Can these identities actually fail? — **audited, because most of them could not**

**`0088` §2(d) strikes an overstated sentence** — ~~*"The run asserts this, so a report that omitted a population could not be written by this pipeline"*~~ — **as a control asserted to exist**, on the ground that a coverage identity of the form `cover(unit, pop, N, N)`, where the population size and the asserted count are **the same expression**, cannot detect an invariant run on a population other than the one named. **Two further shapes of the same defect: a coverage identity written as a literal `True` with no arithmetic behind it, and an aggregate that chains `.get(..., .get(..., .get(..., True)))`, under which an invariant carrying no coverage key at all contributes a pass.**

**Audited on this build.** Every identity is arithmetic on measured counts, and **the population size is sourced from a different file than the asserted count** — the emitted analysis table, the Step 4 ledger, stage 1's own pair count, or the mandated seven-position order. Result on this build: **9 of 9 identities have independently sourced sides**, **0 are literals**, and **9 of 9 invariants carry a coverage identity at all** — with no default: `every invariant must CARRY coverage.identity_holds and it must be True. THERE IS NO DEFAULT: an aggregate that chains .get(..., .get(..., .get(..., True))) lets an invariant with no coverage key contribute a PASS, which is a control that cannot see the thing it was built to see`.
...
**CLAUDE.md -- a control asserted to exist is not a control, and an empty result and a clean result are the same value. A DESCRIPTION of the failure this apparatus would catch proves nothing, so each failure is INJECTED AND EXECUTED here and what is recorded is what the control DID.** The SAME cover(), cover_ok() and _independent_identity() the published invariants go through, and for the aggregate, the same expression the published `identity_holds_on_every_invariant` uses. Not a re-implementation, which would prove only that a second copy behaves.
...
**Why these are executed rather than described.** `CLAUDE.md`: *a control asserted to exist is not a control*, and that file records a property withdrawn because *the mechanism never fired* — found by reading the code rather than the claim. **A field describing the failure this apparatus would catch is a description, not a demonstration.**
...
```json
{
  ...
  "POPULATION_OF_THAT_COUNT": "the APPLY position-5 row set, 196,654 rows -- STATED AT THE POINT OF USE, because this quantity is NOT population-invariant (decisions/0092 Sec 3) and it is emitted on every population below",
...
```json
{
  ...
  "rows_asserted_note": "the two clauses are asserted on disjoint row sets and their sizes are measured separately; the sum is compared against the EMITTED table's row count, which is where a population mismatch would show",
  "why_the_numerator_and_the_null_clause_MUST_share_a_population": "decisions/0080 Sec 3. Asserting the range clause on the POST-LIVENESS Started-and-left rows while the null clause runs on the position-5 remainder leaves the started-and-left liveness exclusions covered by NEITHER clause, and the two printed counts still look right. Both clauses run on position 5 here and the identity is asserted",
