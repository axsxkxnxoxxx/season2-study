...
| git HEAD at launch | `ec340dd`, worktree dirty: True |
| parameters | `W` = 108 d, `H` = 91 d, `τ_pull` = 2026-08-11T00:00:00Z, filter order `decisions/0029`, liveness ALT-BROAD |
| stage files (sha256, 12) | `step8_a_lib.py` cf120b83a386, `step8_a_1_scan.py` a842b073640e, `step8_a_2_positions.py` 9f08ac985c8c, `step8_a_3_table.py` 5b04a7b40ef9, `step8_a_4_arms.py` 3d533b88a113, `step8_a_4b_slugs.py` 4495f8069adb, `step8_a_5_diagnostics.py` 3964a9195191, `step8_a_6_emit.py` 44428c39536c, `step8_a_run.py` 4e6e8a12570a |
...
### 3.1 `p_at_bound` — the split of the `p = 1.0` rows, and why one class is empty

**`decisions/0082` adds `p_at_bound`, a boolean separating the two meanings of `p = 1.0`**: the pair **left at the final episode**, or **the abandonment point is at its bound** — the rank numerator having saturated at `L2`. **Step 10 publishes the abandonment distribution off `abandonment_point_p`, so a spike at 1.0 must be separable.** The ruling requires the split on both populations, summing to the totals already published.

| Population | `p = 1.0` rows | at the bound (numerator = `L2`) | at the final episode | sum |
| :--- | ---: | ---: | ---: | ---: |
| APPLY, position 5 | 1,246 | 1,246 | 0 | 1,246 |
| APPLY, position 7 | 1,230 | 1,230 | 0 | 1,230 |

*Build: every figure in this table measured on `a/2026-08-16` — position-5 build of 2026-08-16, instance `a`; see §0.*

**REPORTED, NOT RESOLVED: the two classes are provably coextensive, so the column cannot separate them.** Under the set-membership rule `A_H ⊆ E2`, so `m_H ∈ E2`, and therefore `|{e ∈ E2 : e ≤ m_H}| = L2` **if and only if** `m_H = max(E2) = F2` — which *is* "left at the final episode". **The two meanings the ruling names are the same event.** One class holds every `p = 1.0` row and the other holds none.

**Measured rather than argued**, on APPLY position 5: **1,246** rows are in both classes, **0** saturated but not at the final episode, **0** at the final episode but not saturated, and **0** in neither. On this frame it is stronger still: **0 of 1,138 shows have any S2 numbering gap**, so `E2 = {1…L2}` everywhere, `F2 = L2`, and the rank form reduces to `m_H / L2` identically.

**The column is emitted as specified and the ruling's requirement is met** — the split is reported and the two classes sum to the published totals. **What is reported and not fixed is that the encoding carries no information beyond `p = 1.0` itself**: `p_at_bound` is `TRUE` exactly on the `p = 1.0` rows, `FALSE` on the rest of Started-and-left, and null elsewhere. **Step 10 cannot use it to tell the two viewer behaviours apart, because under the adopted rank form they are not two behaviours.**
...
**The records-examined denominator: which reading this instance produces, and why.** `decisions/0074` ruling 4 published **6,065,704 against 6,065,610, both reporting 0 drops**, **unreconciled** and routed to Step 14. **It is still unruled.** This instance produces **6,065,704 — the NO-D11 reading** — and states the other two with the arithmetic that separates them, so the choice is visible rather than silent.

| Reading | Records examined | Drops |
| :--- | ---: | ---: |
| **no D11 — produced here** | **6,065,704** | 0 |
| D11 on the S2 side only | 6,065,610 | — |
| D11 on both sides | 6,065,537 | — |

*Build: every figure in this table measured on `a/2026-08-16` — position-5 build of 2026-08-16, instance `a`; see §0.*

**The gap decomposes exactly.** D11 discards **167** in-frame S1/S2 records, of which **94 are S2-side and 73 are S1-side.** So the three readings differ by **where D11 is applied and nowhere else**: 6,065,704 with none, 6,065,610 with D11 on the S2 side, 6,065,537 with D11 on both. **The published 94 is exactly the S2-side count.** Every other candidate axis is zero and was measured, not assumed:
...
**Is it closable? Yes — on the arithmetic above, and this instance says so rather than leaving it as a Step 14 limitation.** The three readings are not three measurements of one quantity that disagree; they are **three different quantities, exactly identified**, and the 94/73 split says which is which with nothing left over. **A ruling naming the reading closes it completely**; a Step 14 limitation would publish as *"the arms disagreed and we do not know why"*, which is not the case — **the cause is known and is one sentence long.** **This instance does not adopt its own answer**: the reading it produces is stated, the reason is given, and the choice remains the Human Lead's. **Reported, not reconciled** (`CLAUDE.md`).
...
- **`p_at_bound` — NEW at `decisions/0082`**, separating the two meanings of `p = 1.0`. Emitted as a **nullable** boolean: null where `p` is null, because an inapplicable value and a false one must not look alike. **The split it is meant to expose is reported in §3.1 — and one of its two classes is empty, provably**, which is reported there and not resolved.

**Two names stay dropped and both are free** (`0080` §2, unchanged by `0081` and `0082`): **`max_episode_in_A`**, read by nothing downstream, and **`f2_in_A_H`**, derivable as `max_episode_in_A_H == s2_F`. **`f2_in_A_H` closes the item this instance reported unreconciled two runs ago**, where `0077` listed the name and fixed a count that could not both be met.
...
13. **`p_at_bound`'s two classes are not two classes.** The spec defines them as distinct meanings of `p = 1.0`; under the adopted rank form and the set-membership rule they are the same event, so the column is `TRUE` exactly where `p = 1.0`. **Emitted as specified, split reported, coextensiveness measured and stated** — §3.1. **Reported, not resolved:** resolving it is a spec decision about what Step 10 needs, not an implementation choice.
...
4. **The 94-record denominator is OPEN and is published unreconciled — but it is CLOSABLE, and §5.1 gives the arithmetic that closes it.** The three readings differ only in where D11 is applied: D11 discards 167 in-frame S1/S2 records, **94 S2-side and 73 S1-side**, so 6,065,704 / 6,065,610 / 6,065,537 are no-D11 / S2-side-only / both-sides, and **the published 94 is exactly the S2-side count.** This instance produces the no-D11 reading and says why. **The choice is the Human Lead's and is not taken here.**
...
8. **Two residuals in the column-set bullets, on both surfaces, both reported and neither load-bearing.** **(a)** The strike-through note is **one revision stale**: `task-sheet.md` Step 8 strikes *"The table is 89 columns"* as *"replaced by the ENUMERATION above (`0080`, extended to 88 by `0081`)"*, and the `analytics-engineer` file as *"replaced by the 88-name ENUMERATION"* — **but the enumeration immediately above is 89 names**, `0082` having added `p_at_bound` after `0081` set it at 88. **(b)** `0077`'s adopted-name table, quoted in both files, **still lists `f2_in_A_H` as an adopted name** — while a bullet in the same section drops it as derivable. **The enumeration is the operative object in both cases and this instance followed it**, asserting set equality against the 89 names; no figure depends on either residual. **Reported, not edited**: neither file is this instance's to amend. Both are the shape `0081` §2 fixed — a superseded count standing beside its replacement — now surviving inside the withdrawal note and the quoted table rather than in live prose.
9. **The set-membership records-examined denominator is still unruled**, and the readings differ by where D11 is applied rather than by anything about the rule. See §5.1: this instance reports the **no-D11** reading with the full decomposition, and states the two other readings and the reason for the choice.
